...
RGB_4_max3loop.py
--------------------------------------------------------------------
Explore the RGB ring pipeline at n=4 with a filter that keeps any
...
def main(*, n: int = 4) -> None:
    ...
    print(f"PrepLP initialized for RGB max3loop n={n}; materializing LP inputs before Mosek.")
    _ = prep.global_keys
    print(
        f"Filtered LP inputs ready for RGB max3loop n={n}: "
    ...
    print(f"Relaxed LP status for RGB max3loop n={n}: {relaxed_solution['status']}")
    print(
        f"Feasible within tolerance for RGB max3loop n={n}: {relaxed_solution['success']}. "
    ...
    if relaxed_solution["success"]:
        print("The filtered RGB n=4 problem is COMPATIBLE.")
    else:
        print("The filtered RGB n=4 problem is detected as INCOMPATIBLE.")
