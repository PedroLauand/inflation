...
from inflation.symmetry_utils import (discover_distribution_symmetries,
                                      discovery_symmetries_from_predicate,
                                      discover_stabilizing_subgroup,
...
class TestSymmetry(unittest.TestCase):
    ...
    def test_discover_stabilizing_subgroup_bsgs(self):
        identity = np.array([[0, 1, 2]], dtype=int)
        # S3 generated by transposition (0 1) and cycle (0 1 2)
        candidate_generators = np.array([[1, 0, 2], [1, 2, 0]], dtype=int)

        subgroup_elements, subgroup = discovery_symmetries_from_predicate(
            stabilizer_predicate=lambda perm: perm[0] == 0,
            initial_generators=identity,
            candidate_generators=candidate_generators,
            verbose=False,
            return_group=True,
        )
        self.assertEqual(int(subgroup.order()), 2,
                         "Stabilizer subgroup should have order 2.")
        self.assertSetEqual(set(map(tuple, subgroup_elements)),
                            {(0, 1, 2), (0, 2, 1)},
                            "Failed to discover the expected stabilizer subgroup.")

        subgroup_group_legacy, subgroup_elements_legacy = discover_stabilizing_subgroup(
            stabilizer_predicate=lambda perm: perm[0] == 0,
            initial_generators=identity,
            candidate_generators=candidate_generators,
            verbose=False,
        )
        self.assertEqual(int(subgroup_group_legacy.order()), 2)
        self.assertSetEqual(set(map(tuple, subgroup_elements_legacy)),
                            {(0, 1, 2), (0, 2, 1)})

    def test_composition_only_stabilizers_are_found(self):
        identity = np.array([[0, 1, 2]], dtype=int)
        # S3 generators, both odd permutations.
        candidate_generators = np.array([[1, 0, 2], [0, 2, 1]], dtype=int)

        def _is_even(perm: np.ndarray) -> bool:
            inversions = 0
        ...
        subgroup_elements, subgroup = discovery_symmetries_from_predicate(
            stabilizer_predicate=_is_even,
            initial_generators=identity,
            candidate_generators=candidate_generators,
            verbose=False,
            return_group=True,
        )
        self.assertEqual(int(subgroup.order()), 3)
        self.assertSetEqual(
            set(map(tuple, subgroup_elements)),
            {(0, 1, 2), (1, 2, 0), (2, 0, 1)},
        )
