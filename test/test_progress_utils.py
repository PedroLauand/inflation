...
class TestProgressUtils(unittest.TestCase):
    ...
    def test_prep_lp_progress_uses_stdout_and_reports_finalization_and_cache_status(self):
        ...
        try:
            ...
            self.assertIn("pilot_size=", stdout_output)
            self.assertIn("pilot_estimator_scale=clip(1.027523 - 2.435372*rho + 2.416697*rho^2, 0.01, 1.0)", stdout_output)
            self.assertIn("initial_per_worker_peak_upper=", stdout_output)
            self.assertIn("initial_active_peak_upper=", stdout_output)
            self.assertIn("Initial row memory upper-bound tally:", stdout_output)
            self.assertIn("upper-bound row peak assumes no compression", stdout_output)
            self.assertIn("marginal size", stdout_output)
            self.assertIn("loop of", stdout_output)
            self.assertIn("Initial active-worker upper bound:", stdout_output)
            ...
            self.assertIn("estimated_compression=", stdout_output)
            self.assertIn("final_compression=", stdout_output)
